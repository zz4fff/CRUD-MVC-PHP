...
class LivroModel {
  ...
  private function conexao() {
    $this->mysqli = new mysqli(BD_SERVIDOR, BD_USUARIO, BD_SENHA, BD_LivroModel);
    if (!$this->mysqli->ping()) {
      printf("Error: %s\n", $this->mysqli->error);
    }
  }
  ...
  public function deleteLivro($id) {
    // TODO: Inserir confirmação de exclusão aqui
  ...
  public function pesquisaParteDeLivro($parte_do_nome) {
    $result = $this->mysqli->query("SELECT * FROM `livros` WHERE `nome` LIKE '%$parte_do_nome%';");
    return $result->fetch_array(MYSQLI_ASSOC);
  }
