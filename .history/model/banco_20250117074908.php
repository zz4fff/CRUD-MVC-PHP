...
$stmt->b    ind_param("sssss", $nome, $autor, $quantidade, $preco, $data);
if( $stm  t->execute() == TRUE){
return t    rue;
} else  {
return false;
}
...
$s  tmt = $this->mysqli->prepare("UPDATE 'livros' SET 'nome' = ?, 'autor'=?,  'q    uantidade'=?, 'preco'=?, 'flag'=?,'data' = ? WHERE 'nome' = ? ");
$stmt->bind_param("sssssss", $nome, $autor, $quantidade, $pr    eco, $flag, $data, $id);
